...
34. **(v0.5 Sprint 2B, 2026-07-10)** A **sidebar passa a ser a navegação oficial do produto**, com os itens Performance Comercial, Analítico Comercial, Analítico Veículos e, temporariamente, 🔧 Auditoria (removida junto com a ferramenta ao fim da conciliação). Supersede a navegação em `st.tabs` registrada no documento 03. Nenhuma ação (botão) vive na sidebar — ações vivem no Masthead.
...
Documentação oficial consolidada na versão 0.4 (ver 06_CHANGELOG.md). Sistema em produção no Streamlit Community Cloud: camada de dados (loader, cleaning, quality_checks, metrics), interface (gate de senha, componentes e as 3 páginas) e ferramenta temporária de auditoria (conciliação de Vendas com a planilha real). Regras vigentes: indicadores Vendas/Faturado/Em Aberto com 8 status oficiais (v0.3) e critério temporal oficial MÊS (VEICULAÇÃO) com YTD verdadeiro (v0.4). Suíte com 117 testes automatizados. Pendências conhecidas: remoção da aba de auditoria ao fim da conciliação; modelagem futura do conceito de PI (contagem, impacto no Ticket Médio e no card Campanhas) registrada para versão futura.
